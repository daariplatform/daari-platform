...
- ✅ **أُغلِق 26 بنداً كلياً + تقدّم جزئي آمن في M‑S7/M‑S4 (بلا مسّ عقد الـ API، بتحقّق؛ من الـ26: 24 بلا لمس schema + 2 تغييرات schema متوافقة رجعياً):**
  ...
  - **أمن (schema، متوافق رجعياً):** `M‑B7` إبطال refresh بعد إعادة تعيين كلمة السر/الخروج/الإبطال الإداري صار يُرفض **حتى داخل نافذة السماح (30ث)** — عمود `revokedReason` يميّز التدوير العادي (يُكرَّم) عن الإبطال الأمني (يُرفض)، فمهاجم بتوكن مسروق لا يُجدّد بعد إعادة تعيين الضحية. ⚠️ **يتطلّب `db push`** (مؤتمَت في `deploy.sh`؛ يُطبَّق مع `clientRequestId` على قاعدة فارغة في أوّل نشر — لا ترحيل بيانات) · `M‑B13` تحويل `onDelete` للدفاتر المالية السبعة (Subscription/RefillOrder/CashHandover/Expense/SalaryPayment/Invoice/WalletTopup) من `Cascade` إلى `Restrict` — حذف مستأجِر/سائق لن يمحو المال بصمت (كامن اليوم؛ تصلّب مستقبلي). تحقّق: `prisma validate`/`generate` + `tsc` = 0.
  - **أمن الويب (إصلاح جزئي آمن، بلا قرار):** `M‑S7` إيقاف تخزين توكن `refresh` الميت في localStorage باللوحتين (يُكتَب ولا يُقرأ أبداً — تقليص سطح XSS بلا تغيير سلوكي؛ يبقى نقل التوكنات لكوكيز `httpOnly` = قرار معماري) · `M‑S4` مهمّة `@Cron` كل ساعة تحذف ملفّات التقارير المنتهية من `uploads/reports/` فقط (تقصر نافذة تعرّض PII، لا تمسّ `proof/`؛ يبقى تقديمها خلف مصادقة = قرار منتج). تحقّق: `tsc` الخادم/اللوحة = 0.
...
- ⏸️ **المتبقّي المفتوح (10 متوسط + المنخفضة):** بنود schema/بنيوية (M‑B5 فهرس فريد جزئي — يحتاج SQL خام لا يناسب `db push` التعريفي · M‑B14 `phone` عالمي هيكلي يمسّ الدخول)، ومسار الإنتاج (M‑T3)، وإعادة هيكلة أكبر/متشابكة (M‑A1/A2 · M‑Q3 تفكيك `complete()` · M‑Q5 كتالوجات الأسعار [تحتاج القيم الصحيحة] · M‑Q10..Q12 [توحيد ودجات Flutter — يتطلّب نقل مجموعة الودجات المشتركة للقلب أولاً، أثر واسع]). **ملاحظة: M‑S4/M‑S7 انتقلتا من «مفتوح» إلى «جزئي» بعد إصلاحيهما الأدنى الآمن — المتبقّي فيهما قرار منتج/معماري فقط.** الكل موثَّق في بانر المطابقة بـ[`AUDIT_REPORT.md`](AUDIT_REPORT.md).
...
- ✅ **تجهيز مسار النشر (بقرارك «نتّجه للنشر»):**
  - أُصلح **فخّ النشر الأوّل** في [`deploy.sh`](deploy/deploy.sh): فحص `/root/PROJECTS.md` كان يُجهِض على خادم مخصّص جديد ويُحيل لملفّ **محذوف** (`PROJECTS-MD-ENTRY.md`) → صار **يتخطّى الفحص تلقائياً على خادم مخصّص** (رسالة مضمّنة على المشترك + تجاوز `SKIP_REGISTRY_CHECK=1`).
  - صُحِّح مسار مفتاح Firebase الافتراضي في [`.env.production.example`](backend/.env.production.example) إلى `/etc/daari-water/` (كان داخل مجلّد النشر فيمحوه `rsync --delete` عند كل نشر — تناقض مع تحذير القالب نفسه).
  - أُنشئ **[`DEPLOY_GUIDE_AR.md`](DEPLOY_GUIDE_AR.md)** — دليل نشر مبسّط بالعربية خطوة-بخطوة (خادم → متطلّبات → nginx → `vps-bootstrap` → `.env` → `deploy.sh` → SSL → تحقّق → اختبار ميداني + استكشاف أخطاء). تحقّق: `bash -n deploy.sh` سليم.
...
1. **(⚠️ يحتاج الخادم الجديد) أوّل نشر متكامل:** **📘 دليل مبسّط خطوة-بخطوة: [`DEPLOY_GUIDE_AR.md`](DEPLOY_GUIDE_AR.md).** أوّل `./deploy/deploy.sh api` **يطبّق `prisma db push`** (عمود/قيد `clientRequestId` [6ب] + عمود `revokedReason` [M‑B7] + قيود `onDelete=Restrict` للدفاتر المالية [M‑B13] — جلسة #20) **+ يزرع مالك المنصّة** (`seed-prod.cjs`، الحاجز #4) **+ ينشئ مجلّد uploads** — كلها مؤتمتة الآن. التغيير غير هدّام (عمود nullable + فهرس على صفوف كلها NULL). الـ runbook لقيد idempotency: [`deploy/RUNBOOK-clientRequestId-db-push.md`](deploy/RUNBOOK-clientRequestId-db-push.md) (نسخة احتياطية → معاينة SQL → تطبيق → تحقّق → تراجع).
...
| # | التاريخ | الخلاصة |
|---|---|---|
| 20 | 2026-07-02 | **مطابقة AUDIT مع `main` + إغلاق 25 بنداً متوسطاً.** حُسم تعارض «AUDIT يعرض الحرجة مفتوحة / PROGRESS يقول مُصلَحة»: AUDIT لقطة قبل #19؛ تحقّق مباشر أنّ C‑1..C‑4 وحذوفات §5/§6 كلها في `main`. workflow بـ**57 عاملاً** على المتوسطة → **44 مُصلَح/3 جزئي/10 مفتوح** (بعد إغلاق هذه الجلسة). أُغلِق 26: `M‑B1` (عمولة REFILL فقط) · `M‑B9` (dedup تذكير) · `M‑B10` (تعليق المتجاوز + عتبات مدى) · `M‑S5` (منع تصعيد `seed-prod`) · `M‑T1` (حارس `truncate`) · `M‑B6/B11` (تسعير + idempotency البثّ) · `M‑B24` (push idempotent) · `M‑B26` (desugaring admin — كان يكسر البناء) · `M‑B27/B28/B29` (queries/معرّف iOS/allowBackup) · `M‑B22/B23` (إيقاف GPS عند انتهاء الجلسة + تزامن الطابور) · `M‑Q9` (logrotate) · `M‑B17/B20` (أخطاء الويب) · `M‑Q1/Q2/Q4/Q7` (تنظيف: تجزئة موحّدة/إيميل ميت/خريطة لترات/إزالة casts) · `M‑B2/B18` (ترقيم الحركات المالية + لوحة الطلبات kanban) · `M‑Q6` (توحيد دوال التقارير بين الشاشة والتصدير) · `M‑B7` (إبطال refresh بعد إعادة التعيين/الخروج حتى داخل نافذة السماح — عمود `revokedReason`) · `M‑B13` (دفاتر مالية `Cascade`→`Restrict`) — **الاثنان يتطلّبان `db push`** مؤتمَت · **جزئيان آمنان:** `M‑S7` (إيقاف تخزين refresh الميت في localStorage) + `M‑S4` (مهمّة تنظيف ملفّات التقارير المنتهية). تحقّق: `tsc`×2=0 · `prisma validate`/`generate` سليم · `flutter analyze` (السائق+core) نظيف · `node --check`/`bash -n` سليمان. المتبقّي 10 متوسط (schema/قرار/إعادة هيكلة أكبر) موثَّق ببانر المطابقة في [`AUDIT_REPORT.md`](AUDIT_REPORT.md). **+ تجهيز نشر (بقرار «نتّجه للنشر»):** إصلاح فخّ `PROJECTS.md` في `deploy.sh` + مسار Firebase في القالب + دليل مبسّط [`DEPLOY_GUIDE_AR.md`](DEPLOY_GUIDE_AR.md). |
