...
- ✅ **أُغلِق 21 بنداً مفتوحاً (بلا مسّ عقد الـ API، بتحقّق):**
...
- ⏸️ **المتبقّي المفتوح (17 متوسط + المنخفضة):** بنود schema تحتاج `db push`/قراراً (M‑B5 فهرس سباق · M‑B7 عمود `revokedReason` · M‑B13 `Cascade` مالي · M‑B14 `phone` عالمي)، وأمن يحتاج قراراً (M‑S4 تقارير PII بلا مصادقة · M‑S7 توكنات في localStorage)، وترقيم/kanban (M‑B2 · M‑B18)، وإعادة هيكلة جودة أكبر (M‑A1/A2 · M‑Q3 تفكيك `complete()` · M‑Q5 كتالوجات الأسعار · M‑Q6 · M‑Q10..Q12) + M‑T3. الكل موثَّق في بانر المطابقة بـ[`AUDIT_REPORT.md`](AUDIT_REPORT.md).
...
| # | التاريخ | الخلاصة |
|---|---|---|
| 20 | 2026-07-02 | **مطابقة AUDIT مع `main` + إغلاق 21 بنداً متوسطاً.** حُسم تعارض «AUDIT يعرض الحرجة مفتوحة / PROGRESS يقول مُصلَحة»: AUDIT لقطة قبل #19؛ تحقّق مباشر أنّ C‑1..C‑4 وحذوفات §5/§6 كلها في `main`. workflow بـ**57 عاملاً** على المتوسطة → **39 مُصلَح/1 جزئي/17 مفتوح** (بعد إغلاق هذه الجلسة). أُغلِق 21: `M‑B1` (عمولة REFILL فقط) · `M‑B9` (dedup تذكير) · `M‑B10` (تعليق المتجاوز + عتبات مدى) · `M‑S5` (منع تصعيد `seed-prod`) · `M‑T1` (حارس `truncate`) · `M‑B6/B11` (تسعير + idempotency البثّ) · `M‑B24` (push idempotent) · `M‑B26` (desugaring admin — كان يكسر البناء) · `M‑B27/B28/B29` (queries/معرّف iOS/allowBackup) · `M‑B22/B23` (إيقاف GPS عند انتهاء الجلسة + تزامن الطابور) · `M‑Q9` (logrotate) · `M‑B17/B20` (أخطاء الويب) · `M‑Q1/Q2/Q4/Q7` (تنظيف: تجزئة موحّدة/إيميل ميت/خريطة لترات/إزالة casts). تحقّق: `tsc`×2=0 · `flutter analyze` (السائق+core) نظيف · `node --check`/`bash -n` سليمان. المتبقّي 17 متوسط (schema/قرار/إعادة هيكلة أكبر) موثَّق ببانر المطابقة في [`AUDIT_REPORT.md`](AUDIT_REPORT.md). |
