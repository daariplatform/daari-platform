...
**آخر تحديث:** 2026-06-30 · **الجلسة:** #15
...
التالي للزبون: **QA شامل** ثمّ التقاعد. تطبيق السائق **قرب التكافؤ**
(لم يُجرَد بعد). تطبيق الإدارة بـ Flutter **مكتمل التكافؤ الوظيفي** — **14 شاشة** (`apps/admin`:
...
البصمة ميدانياً** (يحتاج جهازاً). التركيز التالي: سدّ فجوات الزبون/السائق + QA، أو عناصرك البشرية (القسم 5).
...
1. **(لا يحتاج حسابك)** **جرد تكافؤ تطبيق السائق** (`apps/driver` ↔ `mobile-worker/`) بنفس أسلوب الـ workflow متعدّد الوكلاء (تدقيق ← تحقّق خصومي)، ثم إغلاق الفجوات دفعاتٍ. **الأهمّ:** ⚠️ **QA طابور المال غير المتصل** (راجع القسم 7 — منع double-claim/double-complete).
2. **(يحتاج حسابك — انظر القسم 5)** تجهيز Firebase + توقيع Android، لتمكين أوّل بناء موقّع.
