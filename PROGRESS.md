...
- ⏸️ **مؤجَّل بانتظار قرارك** (موثّق في [`AUDIT_REPORT.md`](AUDIT_REPORT.md) §6): نماذج Prisma غير المرجعية (حذفها يُسقِط جداول إنتاج)، وحدة `vendors` (كود Phase-3)، أدوات Expo/legacy، وتدوير مفتاح Firebase المكشوف في تاريخ git. وبقية المتوسطة/المنخفضة الأقلّ خطورة (إعادة هيكلة تكرار، فهارس/هجرات مخطّط) تُركت للمراجعة المتأنّية.
