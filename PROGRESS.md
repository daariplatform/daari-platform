...
**آخر تحديث:** 2026-07-02 · **الجلسة:** #20
...
| **الجوّال** | **Flutter** هو الواجهة الوحيدة؛ تطبيقات **Expo حُذِفت نهائياً** (جلسة #19). |
...
| **عقد API** | مُجمَّد عموماً. **استثناءان مُقرّان:** قناة FCM (#1)، و**idempotency السلامة المالية** (#16/6ب: حقل `clientRequestId` اختياري + قيد UNIQUE على `RefillOrder`/`CashHandover`). أي تغيير عقد آخر يحتاج قراراً صريحاً. |
| **vendors** | وحدة سوق المورّدين (tuk-tuk) **حُذِفت نهائياً** (جلسة #19) — كانت خلف `FEATURE_VENDORS=false` بلا مستهلك. تُستعاد من تاريخ git إن لزمت مستقبلاً. |

---

## 3) ✅ ما أُنجِز

**جلسة #20 — 2026-07-02 — مطابقة التدقيق مع `main` + إغلاق ذيل متوسط عالي القيمة**
- ✅ **حسم تعارض AUDIT مقابل PROGRESS:** كان `AUDIT_REPORT` يعرض الحرجة الأربع (+كثيراً من المتوسطة) **مفتوحة**، بينما #19 يقول إنها عولِجت — لأنّ AUDIT **لقطة قبل الإصلاحات**. تحقّق مباشر بالكود: **C‑1..C‑4 مُصلَحة فعلاً في `main`** (`next.config.mjs` NetworkOnly · `accounting/page.tsx` تواريخ محلّية · `walkin_screen.dart` `clientRequestId` · `backup-db.sh` تجريد `?schema`)، و**كل حذوفات §5/§6 منفَّذة** (vendors/Expo/نماذج ميتة/deps/كود ميت؛ `PlantGroup` مُبقىً).
- ✅ **تحقّق شامل من الـ57 متوسطاً عبر workflow (57 عاملاً، 0 أخطاء):** **31 مُصلَح / 1 جزئي / 25 مفتوح** — يؤكّد أنّ «المتوسطة عالية القيمة عولِجت» في #19 صحيح. الجدول الكامل في **بانر «مطابقة»** أعلى [`AUDIT_REPORT.md`](AUDIT_REPORT.md).
- ✅ **أُغلِق 23 بنداً مفتوحاً (بلا مسّ عقد الـ API، بتحقّق):**
  - **مالية/سلامة الخادم:** `M‑B1` عمولة السائق تُحسب لطلبات **REFILL فقط** (كانت تشمل tank/walk-in فتنفخ الراتب) · `M‑B9` dedup تذكير/تحذير لكل مستلِم (نافذة تهدئة `REMINDER_COOLDOWN_DAYS`=7) يمنع حرق رصيد WhatsApp/SMS يومياً · `M‑B10` تعليق **أي** مستأجِر متجاوز موعده + عتبات مدى تعاقبية (كان `daysLeft===-1` بالضبط لا يعلّق أبداً عند تشغيل فائت) · `M‑S5` `seed-prod` يرفض ترقية مستخدم مستأجِر إلى `PLATFORM_ADMIN`.
  - **البثّ (WhatsApp):** `M‑B6` جمهور/تسعير البثّ يُحسب مرّة واحدة لكل قناة (كان يُسعّر مجموعة `userId!=null` بينما يُرسِل لمجموعة أوسع → فوترة ناقصة و`sentCount>audienceCount`) · `M‑B11` إثبات إرسال لكل مستلِم على الـjob (BullMQ) فلا يُعاد البثّ المدفوع للجميع عند إعادة المحاولة.
  - **سلامة الاختبارات:** `M‑T1` حارس اسم `_test$` في `truncateAll` (يمنع مسح قاعدة إنتاج/تطوير عبر `npm test`).
  - **Flutter/أصلي:** `M‑B24` `push register()` صار idempotent (المستمعات مرّة واحدة، إعادة إرسال التوكن دائماً) · `M‑B26` **desugaring لتطبيق الإدارة — كان يكسر بناء أندرويد** · `M‑B27` `<queries>` https للإدارة (زرّ التصدير) · `M‑B28` معرّف حزمة iOS للإدارة `daariAdmin→daariadmin` · `M‑B29` `allowBackup=false` للتطبيقات الثلاثة · `M‑B22` إيقاف نبض GPS + تصفير `onShiftProvider` عند انتهاء الجلسة (كانت حلقة 401 تستنزف البطارية) · `M‑B23` حارس تزامن `flush` + `dedupeKey` لتسجيل الزبون.
  - **الويب/النشر:** `M‑B17` `onError` على 3 mutations صامتة (reset سائق/reclaim خزان/حفظ مخزون) · `M‑B20` فرع خطأ لصفحة الاشتراك · `M‑Q9` تثبيت logrotate في `vps-bootstrap.sh`.
  - **تنظيف (بلا تغيير سلوك):** `M‑Q1` توحيد تجزئة كلمة السر عبر `common/crypto` (حذف `ARGON2_OPTS` المكرّر + إعادة `verifyPassword`) · `M‑Q4` خريطة `LITERS_BY_CAPACITY` مشتركة (كانت مكرّرة 4× بافتراضات متناقضة → خطر خصم مخزون خاطئ عند enum جديد) · `M‑Q7` إزالة `as any` عن حقول `Tenant` المُعرَّفة · `M‑Q2` حذف مسار إيصال الإيميل الميت (+ حقن `EmailService` غير المستخدم).
  - **الترقيم/kanban (أخطاء حقيقية):** `M‑B2` ترقيم الحركات المالية — `total` من `count()` حقيقي + سحب `page×pageSize` لكل مصدر (كان `total=merged.length` مسقوفاً فتُخفى صفوف مالية والصفحات البعيدة فارغة) · `M‑B18` لوحة الطلبات تستعلم كل حالة مباشرة (pageSize 200) فتظهر طلبات PENDING الأقدم وتُصبح قابلة للإسناد + عمود «مكتمل اليوم» مُرشَّح بـ`completedAt ≥ اليوم`.
  - **التحقّق:** `tsc --noEmit` الخادم=0 · `tsc --noEmit` اللوحة=0 · `flutter analyze` (السائق+core) نظيف · `node --check` seed-prod · `bash -n` bootstrap. (⚠️ e2e لم يُعَد تشغيله هذه الجلسة: تعديلاتي خارج مسارات e2e المغطّاة، وحارس truncate آمن لـ`maa_platform_test`؛ يُستحسن تشغيله قبل النشر.)
- ⏸️ **المتبقّي المفتوح (15 متوسط + المنخفضة):** بنود schema تحتاج `db push`/قراراً (M‑B5 فهرس سباق · M‑B7 عمود `revokedReason` · M‑B13 `Cascade` مالي · M‑B14 `phone` عالمي)، وأمن يحتاج قراراً (M‑S4 تقارير PII بلا مصادقة · M‑S7 توكنات في localStorage)، وإعادة هيكلة جودة أكبر (M‑A1/A2 · M‑Q3 تفكيك `complete()` · M‑Q5 كتالوجات الأسعار · M‑Q6 · M‑Q10..Q12) + M‑T3. الكل موثَّق في بانر المطابقة بـ[`AUDIT_REPORT.md`](AUDIT_REPORT.md).
- ⚠️ **لم يُلمَس (بقرارك):** تدوير مفتاح Firebase المكشوف في تاريخ git (كونسول — القسم 5)، ووحدة `ai` بلا مستهلك (نقل/حذف)، وعقد الـ API (مُجمَّد).

**جلسة #19 — 2026-07-02 — تدقيق ونظافة شاملة (فرع `audit/cleanup`)**
- ✅ **فحص شامل متعدّد الوكلاء** (167 وكيلًا: فحص حسب البُعد/الوحدة ثم تحقّق عدائي لكل نتيجة وكل عنصر مُرشَّح للحذف). الناتج: [`AUDIT_REPORT.md`](AUDIT_REPORT.md) — 4 حرجة، ~45 متوسطة، ~59 منخفضة + 118 عنصرًا قابلًا للحذف بأمان.
- ✅ **الإصلاحات (من الأخطر للأبسط) — 6 دفعات موثّقة**، كلٌّ متبوعة بـ `tsc`/`nest build`/e2e (18/18) و`next build`×2 و`flutter analyze`:
  - **الحرجة الأربع:** تسريب PII عبر service worker (runtimeCaching=NetworkOnly للـAPI + مسح الكاش عند الخروج) · إزاحة UTC في رواتب السائقين · إسقاط بيعة نقدية ثانية دون اتصال (مفتاح idempotency فريد) · فشل النسخ الاحتياطي الليلي (تجريد `?schema`، استخراج .env بلا source، ملف مؤقّت + فحص سلامة).
  - **متوسطة عالية القيمة:** ربط Postgres/Redis بـ127.0.0.1 + كلمة سرّ Redis (مُوصَّلة أيضًا في cache/queue) · تحقّق إحداثيات الإكمال (GPS anti-fraud) · امتداد الرفع من MIME (منع XSS مخزَّن) · ترويسات أمان (clickjacking) · `cancel()` ذرّي · idempotency الرواتب/الفواتير · OTP تشفيري · مطابقة نقد CASH فقط · حارس خرائط الحالة في اللوحة · إصلاح شريط الموافقات · حماية 401 لصفحة الدخول · وغيرها.
- ✅ **حذف غير المستخدم المؤكَّد:** ~12 اعتمادية خادم + ~10 deps ويب + deps Flutter، ورموز/دوال ميتة (خادم + daari_core + التطبيقات)، وملفّات إعداد ميتة (`.claude/launch.json`, `settings.local.json.bak`)، وإخراج مخرجات `next-pwa` من التتبّع + قواعد `.gitignore`.
- ✅ **تحديث التوثيق:** `STRUCTURE.md` (المخطّط + أعداد الشاشات 18/12/15 + حزمة الويب بلا Zustand)، تنظيف قوالب `.env` من متغيّرات ميتة، وهذا السجلّ.
- ✅ **حذف موسّع بقرار المستخدم:** نماذج Prisma غير المرجعية (DataReport/AdCampaign/BulkDeal + تعداداتها)، **وحدة `vendors` كاملة** (وحدة + خدمة + تحكّم + استعلام المصادقة + قدرة `vendor` + 3 نماذج مخطّط + علاقاتها + `FEATURE_VENDORS`)، و**تطبيقات Expo الثلاثة** (`mobile-customer/worker/admin`) وأدواتها (`eas-setup-and-build.sh`، `generate-icons.py`، `github-setup.sh`). تحقّق بعد كل حذف: `prisma db push` (إسقاط نظيف)، `prisma generate` + `tsc` + `nest build`، **18/18 e2e**.
- ⏸️ **مؤجَّل** (موثّق في [`AUDIT_REPORT.md`](AUDIT_REPORT.md)): تدوير مفتاح Firebase المكشوف في تاريخ git (لا يُعالَج بالحذف)، وبقية المتوسطة/المنخفضة الأقلّ خطورة (إعادة هيكلة تكرار، فهارس/هجرات مخطّط) تُركت للمراجعة المتأنّية. **ملاحظة:** وحدة `ai` صارت بلا مستهلك بعد حذف Expo admin — تُنقَل للوحة Flutter أو تُحذف لاحقاً (قرارك).
...
| # | التاريخ | الخلاصة |
|---|---|---|
| 20 | 2026-07-02 | **مطابقة AUDIT مع `main` + إغلاق 23 بنداً متوسطاً.** حُسم تعارض «AUDIT يعرض الحرجة مفتوحة / PROGRESS يقول مُصلَحة»: AUDIT لقطة قبل #19؛ تحقّق مباشر أنّ C‑1..C‑4 وحذوفات §5/§6 كلها في `main`. workflow بـ**57 عاملاً** على المتوسطة → **41 مُصلَح/1 جزئي/15 مفتوح** (بعد إغلاق هذه الجلسة). أُغلِق 23: `M‑B1` (عمولة REFILL فقط) · `M‑B9` (dedup تذكير) · `M‑B10` (تعليق المتجاوز + عتبات مدى) · `M‑S5` (منع تصعيد `seed-prod`) · `M‑T1` (حارس `truncate`) · `M‑B6/B11` (تسعير + idempotency البثّ) · `M‑B24` (push idempotent) · `M‑B26` (desugaring admin — كان يكسر البناء) · `M‑B27/B28/B29` (queries/معرّف iOS/allowBackup) · `M‑B22/B23` (إيقاف GPS عند انتهاء الجلسة + تزامن الطابور) · `M‑Q9` (logrotate) · `M‑B17/B20` (أخطاء الويب) · `M‑Q1/Q2/Q4/Q7` (تنظيف: تجزئة موحّدة/إيميل ميت/خريطة لترات/إزالة casts) · `M‑B2/B18` (ترقيم الحركات المالية + لوحة الطلبات kanban). تحقّق: `tsc`×2=0 · `flutter analyze` (السائق+core) نظيف · `node --check`/`bash -n` سليمان. المتبقّي 15 متوسط (schema/قرار/إعادة هيكلة أكبر) موثَّق ببانر المطابقة في [`AUDIT_REPORT.md`](AUDIT_REPORT.md). |
