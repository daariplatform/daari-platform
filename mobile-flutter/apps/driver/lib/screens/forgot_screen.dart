...
class _ForgotScreenState extends ConsumerState<ForgotScreen> {
  ...
  Future<void> _requestCode() async {
    ...
    try {
      ...
      if (mounted) {
        showSnack(context, 'أرسلنا لك رمزاً عبر واتساب/رسالة. صالح لمدة 10 دقائق.');
        setState(() => _step = 2);
      }
  ...
  Future<void> _reset() async {
    if (!Validators.isOtp(_otp.text.trim())) {
      setState(() => _otpError = true);
      showSnack(context, 'الرمز يتكوّن من 6 أرقام', error: true);
      return;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_step == 1 ? 'استعادة كلمة السر' : 'تأكيد الرمز'),
      ),
      body: SafeArea(
        child: ListView(
          ...
          children: [
            Icon(
              _step == 1 ? Icons.lock_reset : Icons.sms_outlined,
            ...
            if (_step == 1) ...[
              const Text(
                'أدخل رقمك وسنرسل لك رمزاً عبر واتساب/رسالة لإعادة تعيين كلمة السر.',
              ...
              Text(
                'أرسلنا رمزاً إلى ${_phone.text.trim()}',
