...
## 6) مرشّحات للحذف بانتظار قرارك

هذه لم أحذفها في المرحلة 3 لأنها تتطلّب قرارك:

1. **نماذج Prisma غير المرجعية (§5.3):** حذفها يعني `db push` يُسقِط جداولها من الإنتاج (وأي بيانات فيها). قرارك: حذف نهائي أم نقلها لملفّ roadmap موثّق؟
2. **وحدة `vendors` كاملة (`FEATURE_VENDORS=false`):** كود staged لميزة Phase-3 (موثّق في `.env.production.example:116`)، لكنها في مسار المصادقة الحارّ (استعلام لكل تسجيل دخول). قرارك: حذف أم إبقاء + تسييج استعلام المصادقة خلف نفس العلَم؟
3. **أدوات Expo/legacy المهجورة:** `scripts/eas-setup-and-build.sh` · `scripts/generate-icons.py` · `scripts/github-setup.sh` (الأخير one-shot مُنجَز). تخدم `mobile-customer/worker` المهجورة (لا تزال في المستودع). قرارك: حذفها الآن أم مع حذف تطبيقات Expo لاحقًا؟
4. **مفتاح Firebase المكشوف (low L §أمن الجوّال):** حذف الملفّات لا يزيل المفتاح من تاريخ git؛ يحتاج قرارك حول التدوير/تقييد المفتاح في Google Cloud Console.
