...
## 🔄 مطابقة مع `main` (تحديث — 2026-07-02 · الجلسة #20)

> **كل ما تحت هذا القسم هو لقطة وقت الفحص (قبل الإصلاحات).** المراحل 1→6 (الإصلاح ثم الحذف) **مدموجة في `main`**،
> فأُعيد التحقّق من **كل** بند مقابل كود `main` الحالي (تحقّق مباشر للحرجة والحذوفات + **57 عاملاً متوازياً** للمتوسطة).
> اقرأ الحالة أدناه، لا حالة البند المفرد أدناه في التقرير التاريخي.

**الحرجة الأربع (C‑1..C‑4): ✅ كلها مُصلَحة** — C‑1 `next.config.mjs` (`runtimeCaching: NetworkOnly`) · C‑2 `accounting/page.tsx` (تواريخ محلّية) · C‑3 `walkin_screen.dart` (`clientRequestId` فريد) · C‑4 `backup-db.sh` (تجريد `?schema` + فحص سلامة).

**غير المستخدم (§5) + قرارات الحذف (§6): ✅ منفَّذة بالكامل** — `vendors`، تطبيقات Expo الثلاثة، نماذج Prisma الميتة، deps الخادم/الويب، الكود الميت، سكربتات `prisma:migrate` — و`PlantGroup` مُبقىً (صحيح).

**المتوسطة (57): 44 مُصلَح · 3 جزئي · 10 مفتوح.**
- **✅ مُصلَح (44):** M‑S1 · S2 · S3 · **S5** · S6 · S8 · **B1** · **B2** · B3 · B4 · **B6** · **B7** · B8 · **B9** · **B10** · **B11** · B12 · **B13** · B15 · B16 · **B17** · **B18** · B19 · **B20** · B21 · **B22** · **B23** · **B24** · B25 · **B26** · **B27** · **B28** · **B29** · B30 · **Q1** · **Q2** · **Q4** · **Q6** · **Q7** · Q8 · **Q9** · **T1** · T2 · T4. (الغامق = أُصلح في #20.)
- **◐ جزئي (3):** M‑A3 (حُذفت سكربتات `migrate` فقط؛ لا خطّ هجرات ولا بوّابة `migrate diff` — `db push` غير التفاعلي باقٍ بقرار) · **M‑S7** (أُوقف تخزين توكن `refresh` الميت في localStorage — الحدّ الأدنى الذي أوصى به التدقيق؛ يبقى نقل التوكنات لكوكيز `httpOnly` + CSP كامل = قرار معماري) · **M‑S4** (أُضيفت مهمّة تنظيف تحذف ملفّات التقارير المنتهية فتقصر نافذة PII؛ يبقى تقديم `/uploads/reports/` خلف مصادقة = قرار منتج).
- **⬚ مفتوح (10):** M‑B5 (سباق طلب نشط — يحتاج فهرس فريد جزئي / SQL خام، لا يناسب `db push` التعريفي) · M‑B14 (`User.phone` فريد عالمياً — هيكلي، يمسّ الدخول بالرقم) · M‑T3 (`tsconfig.build.json` — يمسّ مسار الإنتاج). وإعادة هيكلة أكبر/متشابكة: M‑A1 (استخراج خدمات plant) · M‑A2 (حزمة ويب مشتركة) · M‑Q3 (تفكيك `complete()`) · M‑Q5 (كتالوجات أسعار الخطط — تحتاج القيم الصحيحة) · M‑Q10/Q11/Q12 (توحيد ودجات Flutter — متشابكة، تحتاج نقل مجموعة الودجات للقلب أولاً).

**أُصلِح في الجلسة #20 (26 مُغلقاً كلياً + 2 مُتقدَّم جزئياً، بلا مسّ عقد الـ API؛ من الـ26: 24 بلا لمس schema + 2 تغييرات schema متوافقة رجعياً):**
`M‑B1` عمولة السائق تُحسب لطلبات **REFILL فقط** (كانت تشمل tank/walk-in فتنفخ الراتب) · `M‑B9` dedup تذكير/تحذير لكل مستلِم بنافذة تهدئة · `M‑B10` تعليق **أي** مستأجِر تجاوز موعده (كان يتطلّب `daysLeft===-1` بالضبط فالتشغيل الفائت لا يعلّق أبداً) + عتبات مدى · `M‑S5` منع `seed-prod` من ترقية مستخدم مستأجِر إلى مدير منصّة · `M‑T1` حارس اسم `_test` في `truncateAll` (منع مسح قاعدة إنتاج) · `M‑B24` جعل `push register()` idempotent · `M‑B26` desugaring تطبيق الإدارة (**كان يكسر بناء أندرويد**) · `M‑B27` `<queries>` https للإدارة · `M‑B28` معرّف حزمة iOS للإدارة (أحرف صغيرة) · `M‑B29` `allowBackup=false` للتطبيقات الثلاثة · `M‑Q9` تثبيت logrotate في bootstrap · `M‑B17` `onError` على 3 mutations صامتة · `M‑B20` فرع خطأ لصفحة الاشتراك · **البثّ:** `M‑B6` جمهور/تسعير البثّ يُحسب مرّة واحدة (كان يُسعّر مجموعة `userId!=null` بينما يُرسِل لمجموعة أوسع فيُفوتَر ناقصاً و`sentCount>audienceCount`) · `M‑B11` إثبات إرسال لكل مستلِم على الـjob فلا يُعاد البثّ للجميع عند إعادة المحاولة · **السائق:** `M‑B22` إيقاف نبض GPS + تصفير الوردية عند انتهاء الجلسة (كان المؤقّت يرسل 401 في حلقة تستنزف البطارية) · `M‑B23` حارس تزامن `flush` + `dedupeKey` لتسجيل الزبون · **تنظيف (بلا تغيير سلوك):** `M‑Q1` توحيد تجزئة كلمة السر عبر `common/crypto` (حذف `ARGON2_OPTS` المكرّر + إعادة `verifyPassword`) · `M‑Q4` خريطة `LITERS_BY_CAPACITY` مشتركة (كانت مكرّرة 4× بافتراضات متناقضة → خصم مخزون خاطئ عند enum جديد) · `M‑Q7` إزالة `as any` عن حقول `Tenant` المُعرَّفة · `M‑Q2` حذف مسار إيصال الإيميل الميت (+ حقن `EmailService` غير المستخدم) · `M‑Q6` توحيد دوال التقارير (revenue/top-customers/top-drivers) في دوال خاصّة مُعامَلة يستدعيها GET والتصدير معاً (كان التصدير نسخة منفصلة بافتراضات مختلفة) · **الترقيم/kanban:** `M‑B2` ترقيم الحركات المالية: `total` من `count()` حقيقي + سحب `page×pageSize` لكل مصدر (كان `total=merged.length` مسقوفاً والصفحات بعد النافذة فارغة) · `M‑B18` لوحة الطلبات تستعلم كل حالة مباشرة (pageSize 200) فتظهر طلبات PENDING الأقدم من الصفحة الأولى وتُصبح قابلة للإسناد + عمود «مكتمل اليوم» مُرشَّح بـ`completedAt ≥ اليوم` · **أمن (schema):** `M‑B7` إبطال refresh بعد إعادة تعيين كلمة السر/الخروج/الإبطال الإداري يُرفض **حتى داخل نافذة السماح (30ث)** — عمود `revokedReason` يميّز التدوير العادي (يُكرَّم) عن الإبطال الأمني (يُرفض)؛ فمهاجم بتوكن مسروق لا يُجدّد بعد إعادة تعيين الضحية. ⚠️ **يتطلّب `db push`** (مؤتمَت في `deploy.sh`؛ يُطبَّق مع `clientRequestId` على قاعدة فارغة في أوّل نشر — لا ترحيل) · `M‑B13` تحويل `onDelete` للدفاتر المالية السبعة (Subscription · RefillOrder · CashHandover · Expense · SalaryPayment · Invoice · WalletTopup) من `Cascade` إلى `Restrict` — حذف مستأجِر/سائق لن يمحو المال بصمت (كامن اليوم؛ تصلّب مستقبلي). ⚠️ `db push` (`prisma validate`/`generate` = سليم) · **أمن الويب (جزئيان):** `M‑S7` إيقاف تخزين توكن `refresh` الميت في localStorage (تقليص سطح XSS، بلا تغيير سلوكي) · `M‑S4` مهمّة `@Cron` كل ساعة تحذف ملفّات التقارير المنتهية من `uploads/reports/` (تقصر نافذة تعرّض PII، لا تمسّ `proof/`).
**التحقّق:** `tsc --noEmit` الخادم = 0 · `prisma generate` = سليم · `tsc --noEmit` اللوحة = 0 · `flutter analyze` (السائق + core) = نظيف · `node --check` seed-prod = سليم · `bash -n` bootstrap = سليم.

**المنخفضة (§4):** لم تُطابَق فردياً هذه الجلسة (جزء منها عولِج ضمن المرحلة 2؛ الباقي مؤجَّل للمراجعة).
