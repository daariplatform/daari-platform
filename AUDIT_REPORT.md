...
**المتوسطة (57): 41 مُصلَح · 1 جزئي · 15 مفتوح.**
- **✅ مُصلَح (41):** M‑S1 · S2 · S3 · **S5** · S6 · S8 · **B1** · **B2** · B3 · B4 · **B6** · B8 · **B9** · **B10** · **B11** · B12 · B15 · B16 · **B17** · **B18** · B19 · **B20** · B21 · **B22** · **B23** · **B24** · B25 · **B26** · **B27** · **B28** · **B29** · B30 · **Q1** · **Q2** · **Q4** · **Q7** · Q8 · **Q9** · **T1** · T2 · T4. (الغامق = أُصلح في #20.)
- **◐ جزئي (1):** M‑A3 (حُذفت سكربتات `migrate` فقط؛ لا خطّ هجرات ولا بوّابة `migrate diff` — `db push` غير التفاعلي باقٍ بقرار).
- **⬚ مفتوح (15):** أمن/سلامة تحتاج قراراً أو عملاً أكبر: **M‑S4** (تقارير PII بلا مصادقة) · **M‑S7** (refresh في localStorage) · M‑B5 (سباق طلب نشط — يحتاج فهرس/هجرة) · M‑B7 (نافذة refresh بعد التعيين — عمود schema) · M‑B13 (`onDelete: Cascade` مالي — schema) · M‑B14 (`User.phone` فريد عالمياً — schema) · M‑T3 (`tsconfig.build.json` — يمسّ مسار الإنتاج). وإعادة هيكلة (جودة، بلا تغيير سلوك): M‑A1 · M‑A2 · M‑Q3 (تفكيك `complete()`) · M‑Q5 (كتالوجات أسعار الخطط) · M‑Q6 (`gatherReportData`) · M‑Q10 · M‑Q11 · M‑Q12.

**أُصلِح في الجلسة #20 (23 بنداً، كلّها بلا مسّ عقد الـ API وبتحقّق):**
`M‑B1` عمولة السائق تُحسب لطلبات **REFILL فقط** (كانت تشمل tank/walk-in فتنفخ الراتب) · `M‑B9` dedup تذكير/تحذير لكل مستلِم بنافذة تهدئة · `M‑B10` تعليق **أي** مستأجِر تجاوز موعده (كان يتطلّب `daysLeft===-1` بالضبط فالتشغيل الفائت لا يعلّق أبداً) + عتبات مدى · `M‑S5` منع `seed-prod` من ترقية مستخدم مستأجِر إلى مدير منصّة · `M‑T1` حارس اسم `_test` في `truncateAll` (منع مسح قاعدة إنتاج) · `M‑B24` جعل `push register()` idempotent · `M‑B26` desugaring تطبيق الإدارة (**كان يكسر بناء أندرويد**) · `M‑B27` `<queries>` https للإدارة · `M‑B28` معرّف حزمة iOS للإدارة (أحرف صغيرة) · `M‑B29` `allowBackup=false` للتطبيقات الثلاثة · `M‑Q9` تثبيت logrotate في bootstrap · `M‑B17` `onError` على 3 mutations صامتة · `M‑B20` فرع خطأ لصفحة الاشتراك · **البثّ:** `M‑B6` جمهور/تسعير البثّ يُحسب مرّة واحدة (كان يُسعّر مجموعة `userId!=null` بينما يُرسِل لمجموعة أوسع فيُفوتَر ناقصاً و`sentCount>audienceCount`) · `M‑B11` إثبات إرسال لكل مستلِم على الـjob فلا يُعاد البثّ للجميع عند إعادة المحاولة · **السائق:** `M‑B22` إيقاف نبض GPS + تصفير الوردية عند انتهاء الجلسة (كان المؤقّت يرسل 401 في حلقة تستنزف البطارية) · `M‑B23` حارس تزامن `flush` + `dedupeKey` لتسجيل الزبون · **تنظيف (بلا تغيير سلوك):** `M‑Q1` توحيد تجزئة كلمة السر عبر `common/crypto` (حذف `ARGON2_OPTS` المكرّر + إعادة `verifyPassword`) · `M‑Q4` خريطة `LITERS_BY_CAPACITY` مشتركة (كانت مكرّرة 4× بافتراضات متناقضة → خصم مخزون خاطئ عند enum جديد) · `M‑Q7` إزالة `as any` عن حقول `Tenant` المُعرَّفة · `M‑Q2` حذف مسار إيصال الإيميل الميت (+ حقن `EmailService` غير المستخدم) · **الترقيم/kanban:** `M‑B2` ترقيم الحركات المالية: `total` من `count()` حقيقي + سحب `page×pageSize` لكل مصدر (كان `total=merged.length` مسقوفاً والصفحات بعد النافذة فارغة) · `M‑B18` لوحة الطلبات تستعلم كل حالة مباشرة (pageSize 200) فتظهر طلبات PENDING الأقدم من الصفحة الأولى وتُصبح قابلة للإسناد + عمود «مكتمل اليوم» مُرشَّح بـ`completedAt ≥ اليوم`.
**التحقّق:** `tsc --noEmit` الخادم = 0 · `tsc --noEmit` اللوحة = 0 · `flutter analyze` (السائق + core) = نظيف · `node --check` seed-prod = سليم · `bash -n` bootstrap = سليم.
