 ...
 * AsyncStorage so the driver sees today's tasks + history instantly on
 * cold-start, even before the network round-trip completes (or while
 * offline between buildings).
 *
 * Pairs with `<PersistQueryClientProvider>` in app/_layout.tsx.
 *
 * Persistence rules (see `shouldPersistQuery` below):
 *   - Whitelist user-facing reads only (today's tasks, history).
 *   - Mutations are NEVER persisted — replaying a stale write would
 *     double-complete an order. Real offline writes use `lib/offline-queue`.
 *   - Max age 24h — past that we drop the cache to avoid showing
 *     yesterday's tasks at the top of the today list on next launch.
 */
...
const PERSIST_PREFIXES: string[][] = [
  ['driver', 'me'],       // useMyDriverProfile
  ['driver', 'today'],    // useMyTodayTasks
  ['worker', 'history'],  // useMyHistory
];
